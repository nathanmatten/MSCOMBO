...
DiffAbundance <- function(Y, X, Z=NULL, K=NULL, Miss.Mech, ind.samples=NULL, max.miss.perp=0.5, max.miss.image=0.5, method = c("MSNIMBLE", "sIPW", "IPW"), BH.min=NULL, include.updates=T, est.Beta=T, return.nuis=F, refine.C=F, p.refine.both=F, return.all=F, return.mu=F) {
  ...
  if (method=="MSNIMBLE") {
    out.tmp <- Estimation.IPW(Y = Y, Cov = cbind(out$X,out$C.iter,out$Z), Weights = Weights, Var.Weights = Var.Weights, max.miss.C = max.miss.C, ind.int = 1:d, ind.analyze = ind.miss.all | Prob.Missing <= max.miss.C, return.mu=return.mu, return.nuis=return.nuis, method = method, n.quad = 100, Alpha = Alpha, Delta = Delta, t.df.miss = t.df.miss)
  } else {
...
Estimation.IPW <- function(Y=Y, Cov=Cov, Weights, Var.Weights, max.miss.C=0.05, ind.int=NULL, ind.analyze, method=c("IPW","MSNIMBLE"), return.mu=F, return.nuis=F, n.quad=100, Alpha=NULL, Delta=NULL, t.df.miss=4) {
  method <- match.arg(method,c("IPW","MSNIMBLE"))
  n <- ncol(Y)
  p <- nrow(Y)
  r <- ncol(Cov)
  ...
  if (sum(ind.nmar) > 0) {
    tmp.nmar <- lapply(X = which(ind.nmar==T), function(g){ ind.obs <- !is.na(Y[g,])
                  ...
                  if (method=="MSNIMBLE") {
                    out.g <- optim(par=tt$coef, fn=Like, gr=Grad, Y=Y[g,], Cov=Cov, sigma=sqrt(tt$sigma2), alpha=Alpha[g], delta=Delta[g], t.df.miss=t.df.miss, rule=rule.gh, control=list(fnscale=-1), method="BFGS")
                    tt$coef <- out.g$par
                    out.cov.g <- Covn(eta = tt$coef, Y = Y[g,], Cov = Cov, sigma = sqrt(tt$sigma2), alpha = Alpha[g], delta = Delta[g], t.df.miss = t.df.miss, rule = rule.gh, use.empirical = F, return.all = T)
